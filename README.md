...
> Replace `YOUR_GH_USERNAME` and `TF4Swift` with your choices.
...
git init
git add .
git commit -m "Initial commit: TF4Swift core, opgen, and generated ops"
git branch -M main
```
...
```bash
gh repo create YOUR_GH_USERNAME/TF4Swift \
  --public --source=. --remote=origin --push
```

**Option B: manually (HTTPS or SSH)**

```bash
# Create an empty repo on GitHub named TF4Swift (no README/license yet)
# Then set the remote and push:

# HTTPS:
git remote add origin https://github.com/YOUR_GH_USERNAME/TF4Swift.git

# or SSH:
# git remote add origin git@github.com:YOUR_GH_USERNAME/TF4Swift.git
...
- Add a `LICENSE` file (Apache-2.0) and this `README.md` if not already committed.
- Turn on branch protection and CI (GitHub Actions) when ready.
